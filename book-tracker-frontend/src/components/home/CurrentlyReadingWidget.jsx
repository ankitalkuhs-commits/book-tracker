...
function getNudge(maxDays) {
  if (maxDays === null || maxDays < 3) return null;
  if (maxDays < 7) return "Looks like you haven't had much reading time lately — let's change that today! 📖";
  if (maxDays < 14) return "It's been a while! Even 10 pages counts. Your books miss you 😄";
  return "Life gets busy, but your books are patiently waiting. How about a few pages today? ✨";
}
...
export default function CurrentlyReadingWidget({ user, onRoute }) {
  ...
  const maxDays = books.length
    ? Math.max(...books.map(ub => daysSince(ub.updated_at) ?? 0))
    : null;
  const nudge = getNudge(maxDays);
  const buttonLabel = maxDays !== null && maxDays >= 3 ? 'Resume Reading →' : 'Go to Library →';
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {pct !== null && (
                    <div>
                      ...
                      <div style={{ fontSize: '0.75rem', color: '#6B7280', marginTop: '0.3rem', fontWeight: '500' }}>
                        {pct}% · page {ub.current_page} of {book.total_pages}
                      </div>
                    </div>
                  )}
                </div>
          ...
          <button
            ...
            style={{
              marginTop: '0.25rem',
              padding: '0.4rem',
              background: maxDays !== null && maxDays >= 3
                ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)'
                : 'transparent',
              color: maxDays !== null && maxDays >= 3 ? 'white' : '#667eea',
