...
## Requirements
Each requirement cites its triage id. The former **PENDING-4A-CONTRACT** items (R2's server half, R9, R10, R13) are confirmed as assumed by `features/maintenance/sprint-4a-platform-audit/architecture.md` → "Contracts for 4B" (2026-09-13). The build still waits for that 4A backend to be live.
...
- [ ] **R4 — Invite Friends search works (F-05).** `GroupDetailScreen` uses `userAPI.searchUsers`. *Accept:* in a throwaway private circle whose only member is review.reader, typing "review" in the invite box lists review.friend. In Review Circle, review.friend is correctly **not** listed, because existing members are filtered out (tests.md K5).
...
- [ ] **R13 — Tapping a notification marks it read on the server (F-23) — 4A contract confirmed (§3).** An unread row tap calls the 4A per-notification endpoint and refreshes the bell badge. Mark All Read also refreshes the badge. *Accept:* tap one unread notification, relaunch, and it is still read with the badge one lower.
...
- [ ] **R21 — Books without a cover show a placeholder (tests.md K1; follows 4A F-19).** 4A now returns `cover_url: null` when Google has no image. Library → Add Book (search results and the selected book) and a circle's Set Group Book search results show a book icon instead of a blank box. *Accept:* a search that returns a book with no Google image shows the book icon, with no blank tile and no crash.
- [ ] **R22 — Static test suite passes (tests.md).** The pure helpers `src/services/httpPolicy.js` and `src/utils/noteVisibility.js` exist with the exports in architecture T-22. *Accept:* on the merged A+B+C tree, `node --test "__tests__/*.test.mjs"` ends with `# fail 0`.
