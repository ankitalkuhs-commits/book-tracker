...
**Gate G-E1:** `git checkout <deployed sha> && pytest tests/test_books.py -q -k "(book_id or dedup_keys) and not literal_paths and not nulls_group_post"` → **`5 passed, 0 failed`**, collecting exactly the 5 names above (check with `--collect-only -q`).

**Command correction (2026-09-18, measured on the merged A1+A2 tree):** the published `-k` filter collects **6**, not 5. The sixth is **`TestDeleteUserbook::test_delete_nulls_group_post_userbook_id`** — an unrelated F-50 test whose name contains `userbook_id`, and `book_id` matches inside it as a substring. All five required E1 names collect and pass; the extra is benign but makes an exact-count gate unreliable, so every G-E1 command in this plan now also excludes `nulls_group_post` and collects **exactly the five names, 5 passed**. Verified with `--collect-only`. The older correction below (about `test_literal_paths_not_swallowed_by_book_id`) still stands and is already excluded.
...
| G-E1 | 4B add-to-library dedup contract | On the deployed SHA, before any 4B build | `git checkout <deployed sha> && .venv\Scripts\python -m pytest tests/test_books.py -q -k "(book_id or dedup_keys) and not literal_paths and not nulls_group_post"` | **`5 passed, 0 failed`**, collecting exactly T-A2-16a, T-A2-17, T-A2-16, T-A2-23, T-A2-26 (verify with `--collect-only -q`). `6 passed` is accepted only when the sixth name is the pre-existing `TestCatalogAuth::test_literal_paths_not_swallowed_by_book_id` (section 3, "Command correction") | Critical P0 |
...
| **G-E1** | Sprint 4B's add-to-library dedup contract, on the deployed SHA | `git checkout <deployed sha> && .venv\Scripts\python -m pytest tests/test_books.py -q -k "(book_id or dedup_keys) and not literal_paths and not nulls_group_post"` | **`5 passed, 0 failed`**, collecting exactly T-A2-16a, T-A2-17, T-A2-16, T-A2-23, T-A2-26 (`--collect-only -q` to verify). `6 passed` accepted only when the sixth is the pre-existing `test_literal_paths_not_swallowed_by_book_id` |
